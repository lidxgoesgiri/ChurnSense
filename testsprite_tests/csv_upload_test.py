"""Backend verification for POST /api/upload-csv (CSV batch upload).
...
def test_csv_valid_upload():
    ...
    r = requests.post(URL, files={"file": ("data.csv", csv, "text/csv")})
...
def test_csv_empty_rejected():
    r = requests.post(URL, files={"file": ("empty.csv", "", "text/csv")})
    assert r.status_code == 400, f"expected 400, got {r.status_code}: {r.text}"


def test_csv_wrong_header_rejected():
    csv = "name,users,active,churned,revenue\nA,1000,850,150,5000"
    r = requests.post(URL, files={"file": ("bad.csv", csv, "text/csv")})
    assert r.status_code == 400, f"expected 400, got {r.status_code}: {r.text}"


def test_csv_invalid_data_rejected():
    # totalUsers=0 -> schema rejects every row -> no valid rows -> 400.
    csv = f"{HEADER}\nBad,0,0,0,0"
    r = requests.post(URL, files={"file": ("bad.csv", csv, "text/csv")})
    assert r.status_code == 400, f"expected 400, got {r.status_code}: {r.text}"
...
def test_csv_too_large_rejected():
    # >5MB body must be rejected with 413. One data row is ~20 bytes;
    # 300000 rows ~= 6MB, comfortably over the 5MB ceiling.
    large = HEADER + "\n" + ("A,1000,850,150,5000\n" * 300000)
    r = requests.post(URL, files={"file": ("large.csv", large, "text/csv")})
